...
if($action=="probe_query"){
 $query_key=$DB->escape(is_string($_POST['query_key'] ?? null) ? $_POST['query_key'] : '');
 if($query_key==""){
 ...
 $records=[];
 while($rows=$DB->fetch($row)) { $rows['key']=$rows['key_ip']; $records[]=$rows; }
 $encoded=json_encode($records,JSON_HEX_TAG|JSON_HEX_AMP|JSON_HEX_APOS|JSON_HEX_QUOT|JSON_INVALID_UTF8_SUBSTITUTE);
 $data_list_details_probe=substr($encoded,1,-1);
 echo'
